...
impl Server {
    ...
    pub fn new(config: ServerConfig, db: Craton) -> ServerResult<Self> {
        let poll = Poll::new()?;
    ...
    pub fn with_signal_handling(config: ServerConfig, db: Craton) -> ServerResult<Self> {
    ...
    fn process_requests(&mut self, token: Token) {
        use craton_wire::{ErrorCode, Response};
