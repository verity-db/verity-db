//! Request handler that routes requests to Craton.

use craton::{Offset, Craton};
...
/// Handles requests by routing them to the appropriate Craton operations.
pub struct RequestHandler {
    /// The command submitter (wraps Craton with optional replication).
...
impl RequestHandler {
    ...
    /// Creates a new request handler with direct Craton access (no replication).
    #[allow(dead_code)] // Available for direct testing without replication
    pub fn new_direct(db: Craton) -> Self {
        Self {
            submitter: CommandSubmitter::Direct { db },
        }
    }

    /// Returns a reference to the underlying Craton instance.
    pub fn craton(&self) -> &Craton {
        self.submitter.craton()
    }
    ...
    fn handle_inner(&self, request: Request) -> ServerResult<ResponsePayload> {
        let tenant = self.craton().tenant(request.tenant_id);
...
/// Converts wire query parameters to Craton query values.
...
/// Converts a Craton query result to a wire response.
fn convert_query_result(result: &craton_query::QueryResult) -> QueryResponse {
...
fn error_to_wire(error: &ServerError) -> (ErrorCode, String) {
    match error {
        ...
        ServerError::Database(e) => match e {
            craton::CratonError::TenantNotFound(_) => (ErrorCode::TenantNotFound, e.to_string()),
            craton::CratonError::StreamNotFound(_) => (ErrorCode::StreamNotFound, e.to_string()),
            craton::CratonError::TableNotFound(_) => (ErrorCode::TableNotFound, e.to_string()),
            craton::CratonError::PositionAhead { .. } => (ErrorCode::PositionAhead, e.to_string()),
            craton::CratonError::ProjectionLag { .. } => (ErrorCode::ProjectionLag, e.to_string()),
            craton::CratonError::Query(qe) => (ErrorCode::QueryParseError, qe.to_string()),
            craton::CratonError::Storage(_) | craton::CratonError::Store(_) => {
                (ErrorCode::StorageError, e.to_string())
            }
            craton::CratonError::Kernel(ke) => {
