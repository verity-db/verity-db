...
pub enum CommandSubmitter {
    /// Direct mode - applies commands directly to Craton without VSR.
    Direct { db: Craton },
...
impl CommandSubmitter {
    ...
    pub fn new(mode: &ReplicationMode, db: Craton, data_dir: &Path) -> ServerResult<Self> {
        match mode {
    ...
    pub fn submit_with_idempotency(
        ...
        match self {
            ...
            Self::SingleNode { replicator, db } => {
                ...
                // If not a duplicate, apply to Craton for projection updates
                // The replicator handles durability, but Craton manages projections
...
mod tests {
    ...
    use craton_types::{DataClass, Placement, StreamId, StreamName};

    #[test]
    fn test_direct_mode_submit() {
        let temp_dir = TempDir::new().unwrap();
        let db = Craton::open(temp_dir.path()).unwrap();
