//! Main entry point for the Craton SDK.
//!
//! The `Craton` struct provides the top-level API for interacting with Craton.
...
use craton_kernel::{Command, Effect, State as KernelState, apply_committed};
use craton_query::{ColumnDef, DataType, QueryEngine, SchemaBuilder};
use craton_storage::Storage;
use craton_store::{BTreeStore, Key, ProjectionStore, TableId, WriteBatch};
use craton_types::{Offset, StreamId, TenantId};

use crate::error::{Result, CratonError};
use crate::tenant::TenantHandle;

/// Configuration for opening a Craton database.
#[derive(Debug, Clone)]
...
/// The main Craton database handle.
///
...
#[derive(Clone)]
pub struct Craton {
    inner: Arc<RwLock<CratonInner>>,
}

impl Craton {
    /// Opens a Craton database at the given path.
    ///
    /// If the directory doesn't exist, it will be created.
    /// If the database already exists, it will be opened and state recovered.
    pub fn open(data_dir: impl AsRef<Path>) -> Result<Self> {
        let config = CratonConfig::new(data_dir.as_ref());
        Self::open_with_config(config)
    }

    /// Opens a Craton database with custom configuration.
    pub fn open_with_config(config: CratonConfig) -> Result<Self> {
    ...
    pub(crate) fn inner(&self) -> &Arc<RwLock<CratonInner>> {
        &self.inner
    }
}
...
mod tests {
    ...
    use craton_types::{DataClass, Placement, StreamName};
    ...
    fn test_open_creates_directory() {
        ...
        let _db = Craton::open(&db_path).unwrap();
        assert!(db_path.exists());
    }
