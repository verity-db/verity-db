...
fn log_entry_with_idempotency_id() {
    let id = vdb_types::IdempotencyId::generate();
...
fn single_node_effects_include_storage_append() {
    ...
    let stream_id = vdb_types::StreamId::new(0); // Auto-allocated ID
    let append_cmd = vdb_kernel::Command::append_batch(
        stream_id,
...
fn single_node_log_entries_are_sequential() {
    ...
    for i in 1..=10 {
        let cmd = vdb_kernel::Command::create_stream_with_auto_id(
