...
use vdb_query::{QueryResult, Value};
use vdb_types::{DataClass, Offset, Placement, StreamId, StreamName, TenantId};
...
impl TenantHandle {
    ...
    pub(crate) fn new(db: Verity, tenant_id: TenantId) -> Self {
        Self { db, tenant_id }
    }
    ...
    pub fn append(&self, stream_id: StreamId, events: Vec<Vec<u8>>) -> Result<Offset> {
        ...
        let stream = inner
            ...
            .ok_or(VerityError::StreamNotFound(stream_id))?;
    ...
    ///
    /// Note: In `VerityDB`, writes go through the append-only log.
    ...
    pub fn query_at(&self, sql: &str, params: &[Value], position: Offset) -> Result<QueryResult> {
        ...
        let current_pos = vdb_store::ProjectionStore::applied_position(&inner.projection_store);
        if position > current_pos {
            return Err(VerityError::PositionAhead {
                requested: position,
                current: current_pos,
            });
        }
