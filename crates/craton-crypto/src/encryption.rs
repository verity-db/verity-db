...
//!
//! ```
//! use craton_crypto::encryption::{EncryptionKey, Nonce, encrypt, decrypt};
//!
...
//! # Security
//!
//! - **Never reuse a nonce** with the same key. For `Craton`, nonces are
...
///
/// In `Craton`, nonces are derived from the log position to guarantee
...
///
/// Nonces in `Craton` are deterministically derived from the record's log
...
///
/// ```
/// use craton_crypto::encryption::{EncryptionKey, WrappedKey, KEY_LENGTH};
///
/// // KEK wraps a DEK
...
///
/// ```
/// use craton_crypto::encryption::{InMemoryMasterKey, MasterKeyProvider, KeyEncryptionKey};
///
...
impl DataEncryptionKey {
    ...
    ///
    /// ```
    /// # use craton_crypto::encryption::{InMemoryMasterKey, KeyEncryptionKey, DataEncryptionKey, Nonce, encrypt};
    /// # let master = InMemoryMasterKey::generate();
    /// # let (kek, _) = KeyEncryptionKey::generate_and_wrap(&master);
