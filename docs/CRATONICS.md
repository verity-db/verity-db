# Cratonics: The Bedrock Principles

> *"A craton is the ancient, stable core of a continent—unchanged for billions of years while everything around it shifts, erodes, and transforms."*

This document defines the coding philosophy and style for Craton. Like the geological formations we're named after, our code must be **stable**, **enduring**, and **unshakeable**—the bedrock upon which regulated systems can safely build.

Craton is a compliance-first database. Our code must embody the same principles we promise our users: correctness by construction, auditability by design, and no hidden fault lines.
...
We are not building a database for rapid prototyping. We are building a **system of record** for regulated industries—the geological core that everything else builds upon. Every shortcut we take becomes a fault line our users inherit.

- **Do it right the first time.** Refactoring a compliance-critical system is expensive and dangerous.
- **Boring is bedrock.** Clever code erodes under pressure. Obvious code endures.
- **The compiler is your surveyor.** If the compiler can't verify it, neither can auditors.

### Compliance by Construction

Invalid states should be impossible to represent, not merely checked at runtime. If a tenant's data could theoretically leak to another tenant, we have already fractured—even if no bug currently triggers it.
...
- **Audit trails are strata.** Every layer tells a story. If it's not in the log, it didn't happen.

### Simplicity is Structural Integrity

Every abstraction is potential weakness. Every dependency is trust extended. Every line of code is a possible fracture point.

- **Minimize surface area.** Fewer features, fewer faults, fewer CVEs.
- **Prefer std.** The Rust standard library is well-surveyed and stable.
- **Question every dependency.** Does it earn its place in our load-bearing code?
...
**This is a mandatory pattern for all Craton code.** The kernel of Craton is a pure, deterministic state machine—the unchanging core. All side effects live at the edges, like the active margins of a continent.
...
Use Rust's type system to prevent bugs at compile time, not runtime. Build structures that cannot fracture.
...
Validate at system boundaries once, then use typed representations throughout. Like sediment becoming rock through pressure—once compressed, it doesn't need re-validation.
...
Every function should have at least two assertions: one precondition and one postcondition. These are the seismic sensors that detect structural problems before they propagate.
...
Control flow should be visible and bounded. No hidden depths, no unexpected sinkholes.
...
```rust
//! Storage engine for Craton's append-only log.
//!
...
//!
//! ```
//! use craton_storage::{Log, LogConfig};
//!
...
Cratonics is not just a coding style—it's a commitment to building software that endures. Like the ancient continental cores we're named after, our code must withstand the pressures of time, scale, and scrutiny.
...
> *"A craton survives because it doesn't try to be anything other than stable bedrock. Our code should aspire to the same."*
